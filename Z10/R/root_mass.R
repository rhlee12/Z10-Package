...
#' @description This function summarizes the root masses from all live roots in 10 cm depth increments
#'
...
summary.root.mass=function(site){
  ...
  temp=lapply(unique(by.depth$top.depth), function(d) unlist(lapply(unique(by.depth$column), function(c) sum(by.depth$rootDryMass[by.depth$top.depth==d&by.depth$column==c]))))
